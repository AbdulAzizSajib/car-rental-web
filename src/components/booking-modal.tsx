    ...
    <div className="flex-1 flex flex-col items-center justify-center px-8 gap-4 text-center">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          Your booking for{" "}
          <span className="font-medium text-gray-800">{car.name}</span> has been
          placed. We&apos;ll contact you shortly.
        </p>
      </div>
      <div className="w-full rounded-xl border border-gray-100 bg-gray-50 p-4 text-left space-y-2 text-sm">
        ...
        <Row label="Total" value={`৳${totalPrice.toLocaleString()}`} bold />
      </div>
      <Button className="w-full bg-gray-900 text-white hover:bg-gray-800" onClick={onClose}>
        Done
      </Button>
    </div>
  ) : (
        ...
        {STEPS.map((label, i) => (
          <div key={i} className="flex items-center">
            <div className="flex items-center gap-1.5">
              ...
              <span className={`text-xs font-medium transition-colors ${i <= step ? "text-gray-900" : "text-gray-400"}`}>
                {label}
              </span>
            </div>
            {i < STEPS.length - 1 && (
              <div className={`mx-2 h-px w-6 transition-colors ${i < step ? "bg-gray-900" : "bg-gray-200"}`} />
            )}
          </div>
        ))}
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4 custom-scrollbar">
        {step === 0 && (
          <div className="space-y-4">
            ...
            <FieldWrap label="Pickup location" icon={<MapPin size={15} className="text-gray-400" />}>
              <input
                type="text"
            ...
            <FieldWrap label="Drop location" icon={<Navigation size={15} className="text-gray-400" />}>
              <input
                type="text"
        ...
        {step === 1 && (
          <div className="space-y-4">
            <FieldWrap label="Pickup date & time" icon={<Calendar size={15} className="text-gray-400" />}>
              <input
                type="datetime-local"
                min={toMinDate()}
            ...
            <FieldWrap label="Return date & time" icon={<Calendar size={15} className="text-gray-400" />}>
              <input
                type="datetime-local"
            ...
            {days > 0 && (
              <div className="rounded-xl bg-gray-50 border border-gray-100 px-4 py-3 flex items-center justify-between">
                <p className="text-sm text-gray-600">
                  {days} day{days !== 1 ? "s" : ""} · ৳{car.pricePerDay.toLocaleString()} / day
                </p>
                <p className="text-sm font-bold text-gray-900">৳{totalPrice.toLocaleString()}</p>
              </div>
            )}
        ...
        {step === 2 && (
          <div className="space-y-4">
            <FieldWrap label="Contact number" icon={<Phone size={15} className="text-gray-400" />}>
              <input
                type="tel"
            ...
            <FieldWrap label="Special request (optional)" icon={<MessageSquare size={15} className="text-gray-400" />}>
              <textarea
