...
import { deleteBodyTypeAction } from "@/src/services/cars/bodyTypes/deleteBodyType.action";
...
export default function AdminBodyTypesPage() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<BodyType | null>(null);
  ...
  const fetchBodyTypes = useCallback(async () => {
    setLoading(true);
    try {
      const res = await getBodyTypesAction({
        page,
        limit: PAGE_SIZE,
  ...
  const openEdit = (BodyType: BodyType) => {
    setEditing(BodyType);
    setForm({ name: BodyType.name, image: BodyType.image ?? "" });
    setFormError(null);
    setDialogOpen(true);
  };
  ...
  const submitForm = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editing) {
        await updateBodyTypeAction(editing.id, payload);
      } else {
        await createBodyTypeAction(payload);
      }
      setDialogOpen(false);
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      await deleteBodyTypeAction(deleteTarget.id);
      setDeleteTarget(null);
      if (bodyTypes.length === 1 && page > 1) {
        setPage((p) => p - 1);
      } else {
        await fetchBodyTypes();
      }
    ...
    <div className="min-h-screen p-8" style={{ background: "#faf8f4" }}>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>
              {editing ? "Edit Body Type" : "New Body Type"}
            </DialogTitle>
          </DialogHeader>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete body type?</AlertDialogTitle>
            <AlertDialogDescription>
